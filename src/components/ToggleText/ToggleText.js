...
export default class ToggleText extends Component {
    ...
    render() {
        ...
        const shouldTextToBeCropped = !textExpanded && innerChildren.length > maxLength

        return (
            <div className={css.toggleTextWrapper}>
                <CustomTag className={className}>
                    {
                        shouldTextToBeCropped ? innerChildren.slice(0, maxLength) : innerChildren
                    }
                </CustomTag>

                {shouldTextToBeCropped ? <div className={css.blurredLine}></div> : null}

                <button
                    className={`${css.toggleButton} ${textExpanded ? css.toggleButtonExpanded : ''}`}
                    onClick={this.handleTextToggling}>
                    Read {textExpanded ? 'less' : 'more'}
                </button>
            </div>
        )
    }
