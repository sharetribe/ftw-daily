...
const allowedInputProps = (allProps) => {
  ...
  const {
    currencyConfig, defaultValue, intl, input, meta, ...inputProps
  } = allProps
  return inputProps
}
...
class CurrencyInputComponent extends Component {
  ...
  render() {
    const {
      className, currencyConfig, defaultValue, placeholder, intl
    } = this.props
...
const {
  func, oneOfType, number, shape, string, object
} = PropTypes
...
const FieldCurrencyInputComponent = (props) => {
  const {
    rootClassName, className, id, label, input, meta, ...rest
  } = props
