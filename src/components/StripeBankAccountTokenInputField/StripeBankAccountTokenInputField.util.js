...
export const inputTypeToString = (inputType, intl) => {
  if (BANK_ACCOUNT_INPUTS.includes(inputType)) {
    ...
    throw new Error('Unknown inputType given to validator');
  }
};
...
export const translateStripeError = (country, intl, stripeError) => {
  ...
  if (ibanRequired) {
    ...
    const inputsInString = inputsAsStrings.length > 1
      ? inputsAsStrings.join(', ').replace(/,([^,]*)$/, ' and $1')
      : inputsAsStrings[0];
 ...
 */
export const validateInput = (inputType, value, country) => {
  switch (inputType) {
    case ACCOUNT_NUMBER:
      return window.Stripe.bankAccount.validateAccountNumber(value, country);
    case ROUTING_NUMBER:
      return window.Stripe.bankAccount.validateRoutingNumber(value, country);
    case BSB:
    case IBAN:
    case SORT_CODE:
...
export const formatFieldMessage = (intl, inputType, messageType) => {
  if (!BANK_ACCOUNT_INPUTS.includes(inputType)) {
    throw new Error(`inputType must be one of ${BANK_ACCOUNT_INPUTS}`);
  }
