...
const prevMonthFn = (currentMoment) => currentMoment
.clone()
.subtract(1, 'months')
.startOf('month')

// Get a function that returns the start of the next month
const nextMonthFn = (currentMoment) => currentMoment
.clone()
.add(1, 'months')
.startOf('month')
...
const momentToUTCDate = (dateMoment) => dateMoment
.clone()
.utc()
...
const findException = (exceptions, day) => {
  return exceptions.find((exception) => {
    ...
    const { start } = availabilityException.attributes
    const dayInUTC = day.clone().utc()
...
const renderDayContents = (calendar, availabilityPlan) => (date) => {
  ...
  const {
    isOutsideRange, isSameDay, isBlocked, isBooked, isInProgress, isFailed
  } = dateModifiers(
    availabilityPlan,
    exceptions,
    bookings,
...
class ManageAvailabilityCalendar extends Component {
    ...
    if (isMonthInRange(monthMoment, TODAY_MOMENT, END_OF_RANGE_MOMENT)) {
      ...
      if (isMonthInRange(startMoment, TODAY_MOMENT, END_OF_BOOKING_RANGE_MOMENT)) {
        ...
        availability.onFetchBookings({
          listingId, start, end: endForBookings, state
        })
      }
    }
  }
  ...
  onDayAvailabilityChange(date, seats, exceptions) {
    ...
    if (hasAvailabilityException) {
      const { id } = currentException.availabilityException
      ...
      if (isResetToPlanSeats) {
        ...
        .then((r) => {
          const params = {
            listingId, start, end, seats, currentException: exception
          }
  ...
  onDateChange(date) {
    ...
    const { exceptions = [], bookings = [] } = calendar[monthIdStringInUTC(date)] || {}
    const {
      isPast, isBlocked, isBooked, isInProgress
    } = dateModifiers(
      availabilityPlan,
      exceptions,
      bookings,
