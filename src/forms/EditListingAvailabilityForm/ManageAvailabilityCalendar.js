...
const prevMonthFn = (currentMoment) => currentMoment.clone().subtract(1, 'months').startOf('month')

// Get a function that returns the start of the next month
const nextMonthFn = (currentMoment) => currentMoment.clone().add(1, 'months').startOf('month')
...
const momentToUTCDate = (dateMoment) =>
  dateMoment.clone().utc().add(dateMoment.utcOffset(), 'minutes').toDate()
...
const isAfterEndOfBookingRange = (date) =>
  !isInclusivelyBeforeDay(date, END_OF_BOOKING_RANGE_MOMENT)
